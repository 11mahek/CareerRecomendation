...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_profile'])) {
    ...
    if ($isMentor) {
        $stmt = $conn->prepare("UPDATE mentors SET bio=?, profession=?, experience=?, skills=?, availability=? linkedin=?, language=? WHERE email=?");
...
if (isset($_POST['delete_account'])) {
    ...
    if ($isMentor) {
        $stmt = $conn->prepare("DELETE FROM mentors WHERE email=?");
        $stmt->bind_param("s", $email);
        $stmt->execute();
    }
...
<div class="container mt-5">
    ...
    <?php if (!empty($user['profile_pic'])): ?>
        <img src="<?php echo htmlspecialchars($user['profile_pic']); ?>" class="profile-pic"><br><br>
        <?php endif; ?>
